...
public class Base64 {
    ...
    protected static final Log logger = LogFactory.getLog(Base64.class);
    ...
    static void compare(char[] b1, char[] b2) {
        if (b1 == null || b2 == null) {
        	logger.info("Base64.compare(cbar): Null array!");
        } else if (b1.length != b2.length) {
			logger.info("Base64.compare(cbar): arrays are different lengths!");
        } else for (int i=0; i<b1.length; i++) {
            if (b1[i] != b2[i]) {
                logger.info("Base64.compare(char): arrays disagree at byte " + i);
                return;
            }
        }
    }
    ...
    static void compare(byte[] b1, byte[] b2) {
        if (b1 == null || b2 == null) {
            logger.info("Base64.compare(byte): Null array!");
            return;
        }
        if (b1.length != b2.length) {
            logger.info("Base64.compare(byte): arrays are different lengths!");
            return;
        }
        for (int i=0; i<b1.length; i++) {
            if (b1[i] != b2[i]) {
                logger.info("Base64.compare(byte): arrays disagree at byte " + i);
                return;
            }
        }
    }
